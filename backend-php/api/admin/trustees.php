...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($isUpdate) {
            // UPDATE
            // If no new image, keep the old one (fetch current if not provided in input, though input usually has nothing for file fields)
            if ($imageUrl === null) {
                // If we didn't upload a new one, do we have one passed? 
                // Usually not in FormData. So we need to fetch existing to keep it, OR just don't update that column.
                // Better strategy: Dynamic update query.
                ...
                $stmt = $pdo->prepare($sql);
                $stmt->execute($params);
                echo json_encode(['success' => true, 'message' => 'Trustee updated with new image']);
            }
        } else {
            // CREATE
